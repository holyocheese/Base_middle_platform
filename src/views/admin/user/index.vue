<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table :key='tableKey' :data="list" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%;font-size:12px;">
      ...
      <el-table-column width="130" align="left" label="姓名"> <template slot-scope="scope">
        <span>{{scope.row.name}}</span>
      </template> </el-table-column>
      <el-table-column width="120" align="left" label="账户"> <template slot-scope="scope">
        <span>{{scope.row.username}}</span>
      </template> </el-table-column>
      <el-table-column width="80" align="center" label="所属组"> <template slot-scope="scope">
        <el-tooltip placement="bottom" effect="light">
          <div slot="content">组主管：{{scope.row.attr1}}<br/>组成员：{{scope.row.attr2}}</div>
          <el-button size="small">查看</el-button>
        </el-tooltip>
      </template> </el-table-column>
      <el-table-column width="300" align="left" label="备注"> <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作"> <template slot-scope="scope">
        ...
        <el-button v-if="userManager_btn_edit" size="small" type="success" @click="handleChangePass(scope.row)">改密码
        </el-button>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="密码" placeholder="请输入密码" prop="password"
                      v-if="dialogStatus == 'create'|dialogStatus == 'changePass'">
          <el-input type="password" v-model="form.password"></el-input>
        </el-form-item>
        <el-form-item label="确认密码" placeholder="请输入密码" prop="confirmPassword" v-if="dialogStatus == 'changePass'">
...
<script>
  ...
  import {page4out} from 'api/admin/tenant/index';
  ...
  export default {
    ...
    data() {
      ...
      var checkPass = (rule, value, callback) => {
        ...
        if (!reg.test(value)) {
          callback(new Error("密码必须包含数字，字母，特殊字符其中的两种以上！"));
        } else {
          callback();
        }
      };
